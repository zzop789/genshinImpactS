...
enemy::enemy(std::vector<Atlas> &anim_enemy):speed(3),anim_left(nullptr), anim_right(nullptr) {

	anim_left = new animation(&(anim_enemy.at(0)), 45);
	anim_right = new animation(&(anim_enemy.at(1)), 45);
	enum edge
	{
		up = 0,
		down,
...
void enemy::drawPlay(int delta)
{
	...
	//rectangle(collier.center.x - 40, collier.center.y + 40, collier.center.y - 40, collier.center.x + 40);
...
bool enemy::check_paimeng(gameObject& paimeng)
{
	vector2d center = this->trans.postion;
	vector2d distance = paimeng.obj_transform.postion - center;
	if (distance.getLength() < 40 * 1.414) 
		return true;
	return false;
}